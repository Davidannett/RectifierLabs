...
.EXAMPLE
.\ASPS.AutomateBuild.ps1 .\MyProfile.AutomateBuild.xml -TestPassword:$false

Description:
Executes the engine on the XML and disregards validation of credentials. This is useful if you are executing prior to ZID creation or just don't want to check.
...
set-variable -Name DEFAULT_SITE_LOG -Value "d:\data\logs\iis" -Option Constant
#endregion
...
function Tokenize {
    ...
    if($returnValue -is [String]) {
        ...
        $returnValue = $returnValue -replace "##SERVERLAST4DIGITS##",$ComputerName.Substring($ComputerName.Length -4)
        $returnValue = $returnValue -replace "##SERVERLAST3DIGITS##",$ComputerName.Substring($ComputerName.Length -3)
        $returnValue = $returnValue -replace "##SERVERLAST2DIGITS##",$ComputerName.Substring($ComputerName.Length -2)
        $returnValue = $returnValue -replace "##SERVERLAST1DIGITS##",$ComputerName.Substring($ComputerName.Length -1)
...
function GetPassword {
    ...
    if($cache.Contains($account)) {
        ...
        if(-not $Quiet) {
            ...
            If($TestPassword){
                $Validation = TestPassword $accountName $lastPassword $accountDomain
                If($Validation -eq $true){
                    # SUCCESS. Do nothing on purpose
                }
                Else{
                    AbortWithError -message "$accountName validation failed. AutomateBuild has been terminated" -tip "Please ensure ZIDs have necessary logon access via PIMs and that the password provided is accurate."
                }
            }
...
function TestPassword {
    ...
    Begin{
        # Format parameters TOUPPER for consistency
        $UserName = $UserName.ToUpper()
        $Domain = $Domain.ToUpper()
        $Account = "$Domain\$Username"
    }
    Process{
        $Validation = New-Object System.DirectoryServices.DirectoryEntry("LDAP://dc=$Domain,dc=com",$UserName,$Password)
        If($Validation.Path -ne $null -and $Validation.distinguishedName -ne $null){
            Write-Host "SUCCESS: Credential validation passed for $Account on $env:COMPUTERNAME" -ForegroundColor Green
            $Result = $true
        }
        Else{
            Write-Host "ERROR: Credential validation failed for $Account on $env:COMPUTERNAME" -ForegroundColor Red
            $Result = $false
        }
    }
    End{
...
function IISLogFields.Modify {
    param ([string]$mode, [string]$field)
    # get existing IIS log fields
    $preModification = Get-WebConfigurationProperty -Filter system.applicationHost/Sites/SiteDefaults/logfile -name "logExtFileFlags"
    if($mode -eq "add") {
        Log "Needs to be implemented"
        LogVerbose "A smart person needs to work through the usecase for this change"
    }
    elseif($mode -eq "delete") {
        if($preModification -like "*$flag*") {
            $postModification = $preModification -replace "$flag,",""
            Set-WebConfigurationProperty -Filter system.applicationHost/Sites/SiteDefaults/logfile -name "logExtFileFlags" -value "$postModification"
            Log ("SUCCESS: Deleted $flag from IIS Log Flags")
        }
        else {
            LogVerbose "The $flag does not exist in the IIS Log Flags"
        }
    }
}
...
function W3SVCFolder.Create {
    param ([string]$siteName, [string]$folder)
    $getSiteCollection = (Get-WebSite) | foreach {$_} {
        if($_.name -eq $siteName) {
            $id = $_.ID
            if(-not(Test-Path "$folder\W3SVC$id" -PathType Container)) {
                LogVerbose "Creating W3SVC subfolder $folder\W3SVC$id"
                New-Item "$folder\W3SVC$id" -ItemType Directory | Out-Null
            }
        }
    }
}
...
foreach($Password in $Passwords) {
    ...
    if($firstEqualSign -gt 0) {
        ...
        $userName = $account.Substring($account.IndexOf('\')+1)
        $Domain = $Password.Substring(0,$Password.IndexOf('\')).ToUpper()
        ...
        if(-not $cachedPasswords.Contains($account)) {
            # If TestPassword:$true (default), test provided password
            If($TestPassword){
                # If there are remote computers, invoke the command against them
                If($RemoteComputers.Count -gt 0){
                    Log "Validating $account on server(s): $($RemoteComputers -join ',')"
                    # Test against each server. AutomateBuild will exit upon a failed validation.
                    foreach($RemoteComputer in $RemoteComputers){
                        $Validation = Invoke-Command -ComputerName $RemoteComputer -ScriptBlock ${Function:TestPassword} -ArgumentList $userName,$Password,$Domain
                        If($Validation -eq $true){
                            # SUCCESS. Do nothing on purpose
                        }
                        Else{
                            AbortWithError -message "$account validation failed against $RemoteComputer. AutomateBuild has been terminated" -tip "Please ensure ZIDs have necessary logon access via PIMs and that the password provided is accurate."
                        }
                    }
                }
                Else{
                    # No remote computers passed. Run TestPassword
                    $Validation = TestPassword $userName $password $Domain
                    If($Validation -eq $true){
                        # SUCCESS. Do nothing on purpose
                    }
                    Else{
                        AbortWithError -message "$account validation failed. AutomateBuild has been terminated" -tip "Please ensure ZIDs have necessary logon access via PIMs and that the password provided is accurate."
                    }
                }
            }
            ...
            if($attemptAction) {
                ...
                switch ($actionType) {
                        ...
                        elseif(-not $whatIf.IsPresent) {
                            Try {
                                ...
                                if ($identityType -eq 3) {
                                    ...
                                    $password = GetPassword $cachedPasswords $userName -TestPassword:$TestPassword
                    ...
                    "IIS.CreateSite" {
                        ...
                        elseif(-not $whatIf.IsPresent) {
                            Try {
				#Ensure the configuration items for authentication are unlocked.
				...
				Set-WebConfiguration //System.WebServer/Security/Authentication/windowsAuthentication -metadata overrideMode -value Allow -PSPath IIS:/
                                ...
                                if([string]::IsNullOrEmpty($loggingFolder)) {
                                    LogVerbose "Removing site $name logging folder override"
                                    # based on $loggingFolder Param being empty, set log directory back to default
                                    Set-ItemProperty "IIS:\Sites\$name" -name logFile -value @{directory="$DEFAULT_SITE_LOG"}
                                    # create the W3SVC folder to enforce parent security rules
                                    W3SVCFolder.Create "$name" "$DEFAULT_SITE_LOG"                                   
                                }
                                elseif($loggingFolder -ne $null) {
                                    ...
                                    # create the W3SVC folder to enforce parent security rules
                                    W3SVCFolder.Create $name $loggingFolder                                    
                                }
                    ...
                    "IIS.CreateFTPSite" {
                        ValidateParamNames $action @("Name","Folder","ApplicationPool","LoggingFolder","Port","Read","Write","FullControl")
                        $name = GetActionParam $action "Name" $True
                        $folder = GetActionParam $action "Folder" $True
                        $applicationPool = GetActionParam $action "ApplicationPool" $True
                        $loggingFolder = GetActionParam $action "LoggingFolder" $False
                        $port = GetActionParam $action "Port" $False "21"
                        ...
                        Log "  LoggingFolder=$loggingFolder"
                        Log "  Port=$port"
                        foreach($group in $reads) { Log "  Reads=$group" }
                        foreach($group in $writes) { Log "  Writes=$group" }
                        foreach($group in $fullControls) { Log "  FullControls=$fullControls" }
 
                        if(-not (PSCmdletAvailable "New-WebFtpSite")) {
                            LogError "Unable to attempt action; New-WebFtpSite cmdlet not available"
                            $actionSuccess = $whatIf.IsPresent
                        }                                                
                        elseif(-not $whatIf.IsPresent) {
                            Try {
                                ...
                                if(Test-Path "IIS:\Sites\$name") {
                                    # force site properties for application pool and folder
                                    LogVerbose "FTP Site $name already exists, forcing app pool to $applicationPool and physical path to $folder"
                                    Set-ItemProperty "IIS:\Sites\$name" -Name ApplicationPool $applicationPool
                                    Set-ItemProperty "IIS:\Sites\$name" -Name PhysicalPath -Value $folder
                                }
                                else {
                                    LogVerbose "Creating new FTP site $name with app pool $applicationPool and physical path $folder"
                                    # workaround for a bug that exists with New-Website when no site exists in IIS. 
                                    $siteID = (Get-ChildItem 'IIS:\Sites' | ForEach {$_.id} | sort -Descending | Select -First 1) + 1
                                    New-WebFtpSite -Name $name -PhysicalPath $folder -Port $port -Id $siteID -Force | Out-Null
                                    Set-ItemProperty "IIS:\Sites\$name" -Name ApplicationPool $applicationPool
                                }
                                LogVerbose "Setup FTP site authentication"
                                # enable basic authentication to FTP site
                                Set-ItemProperty "IIS:\Sites\$name" -Name ftpServer.security.authentication.basicAuthentication.enabled -Value $true                                
                                # remove all authorization rules as Set-WebConfiguration does not work for FTP site
                                Try {
                                    Remove-WebConfigurationProperty -PSPath 'MACHINE/WEBROOT/APPHOST' -Location "$name" -Filter "/system.ftpServer/security/authorization" -Name "." -AtElement @{users="";roles="*";permissions="*"}
                                }
                                Catch {
                                    LogError ("Unable to reset FTP site authorization rules")
                                    Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                    Log ("  Exception Message: $($_.Exception.Message)")
                                    $actionSuccess = $False
                                }

                                # setup security for groups/users that require read access
                                if($reads -ne $null) {
                                    foreach ($group in $reads) {
                                        Try {
                                            Add-WebConfiguration -PSPath 'MACHINE/WEBROOT/APPHOST' -Location "$name" -Filter "/system.ftpServer/security/authorization" -value @{accessType="Allow";roles="$group";permissions=1}
                                        }
                                        Catch {
                                            LogError ("Unable to create access rule for $group")
                                            Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                            Log ("  Exception Message: $($_.Exception.Message)")
                                            $actionSuccess = $False
                                        }
                                ...
                                # setup security for groups/users that require write access
                                if($writes -ne $null) {
                                    foreach ($group in $writes) {
                                        Try {
                                            Add-WebConfiguration -PSPath 'MACHINE/WEBROOT/APPHOST' -Location "$name" -Filter "/system.ftpServer/security/authorization" -value @{accessType="Allow";roles="$group";permissions=2}
                                        }
                                        Catch {
                                            LogError ("Unable to create access rule for $group")
                                            Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                ...
                                # setup security for groups/users that require read and write access
                                if($fullControls -ne $null) {
                                    foreach ($group in $fullControls) {
                                        Try {
                                            Add-WebConfiguration -PSPath 'MACHINE/WEBROOT/APPHOST' -Location "$name" -Filter "/system.ftpServer/security/authorization" -value @{accessType="Allow";roles="$group";permissions=3}
                                        }
                                        Catch {
                                            LogError ("Unable to create access rule for $group")
                                            Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                ...
                                #setup FTP site level logging if required
                                if([string]::IsNullOrEmpty($loggingFolder)) {
                                    LogVerbose "Removing site $name logging folder override"
                                    # based on $loggingFolder Param being empty, set log directory back to default
                                    Set-ItemProperty "IIS:\Sites\$name" -name ftpServer.logFile -value @{directory="$DEFAULT_SITE_LOG"}
                                    # create the W3SVC folder to enforce parent security rules
                                    W3SVCFolder.Create "$name" "$DEFAULT_SITE_LOG"                                    
                                }
                                elseif($loggingFolder -ne $null) {
                                    ...
                                    LogVerbose "Setting site $name logging folder override to $loggingFolder"
                                    Set-ItemProperty "IIS:\Sites\FTPOnBoard" -name ftpServer.logFile -value @{directory="$loggingFolder"}
                                    # create the W3SVC folder to enforce parent security rules
                                    W3SVCFolder.Create "$name" "$loggingFolder"
                                }
                            ...
                            Catch {
                                LogError ("Unable to create/update FTP site $name")
                                Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                Log ("  Exception Message: $($_.Exception.Message)")
                                $actionSuccess = $False
                            }
                    ...
                    "IIS.RemoveCookieCollection" {
                        ...
                        if(-not $whatIf.IsPresent) {
                            Try {
                                IISLogFields.Modify $mode $flag
                            }
                            Catch {
                                LogError ("Failed to render IIS Log Flags request $name")
                                Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                Log ("  Exception Message: $($_.Exception.Message)")
                                $actionSuccess = $False
                            }
                    ...
                    "Security.AddLocalGroup" {
                        ValidateParamNames $action @("Group","Description")
                        ...
                        if(-not $whatIf.IsPresent) {
                            # add each item to the local account database
				            $winNTcomputer = [ADSI]("WinNT://$ComputerName,computer")
                            if ([ADSI]::Exists("WinNT://./$group")) {
                                LogVerbose "$group already exists"
                            }
                            else {
                                Try {
                                    # first creat the new group
                                    $newGroup = $winNTcomputer.Create("Group","$group")
                                    $newGroup.setinfo()
                                    # second apply the group discription if present
                                    if(-not ([string]::IsNullOrEmpty($group))) {
                                        $newGroup.description = "$description"
                                        $newGroup.setinfo()
                                    }
                                    Log "Success: Added $group"
                                }
                                Catch {
                                    LogError "Failed adding $group to local machine"
                                    Log ("  Exception Type: $($_.Exception.GetType().FullName)")
                                    Log ("  Exception Message: $($_.Exception.Message)")
                                    $actionSuccess = $False
                                }
                    ...
                    "Security.GetValidPassword" {
                        ...
                        if(-not $whatIf.IsPresent) {
                            foreach($account in $accounts) {
                                $getPassword = GetPassword $cachedPasswords $account -TestPassword:$TestPassword
                                if($getPassword -eq "") {
                    ...
                    "Powershell.ExecuteCommand" {
                        ...
                        if(-not $whatIf.IsPresent) {
                            ...
                            foreach($psFile in $psFiles) {
                                ...
                                Try {
                                    Invoke-Command -FilePath $psScript -ComputerName $ComputerName
                    ...
                    "Model3API.AddSwaggerServer" {
                        ...
                        Log        "  This module has been deprecated"
                        LogVerbose "  Swagger 1.2 has been deprecated, please switch to Model3API.AddSwaggServer20"
                        $actionSuccess = $False
                    ...
                    "Model3API.AddAPI" {
                        ...
                        Log "  Swagger 1.2 functionality has been removed and deprecated"
                        Log "  Swagger 2.0 functionality has been removed, please utilize action Model3API.AddSwaggerServer20"
                        ...
                        elseif(-not $whatIf.IsPresent) {
                            Try {
                                foreach($api in $apis) {
                                    ...
                                    $pool.processModel.password = "$(GetPassword $cachedPasswords $poolUserName -TestPassword:$TestPassword)"
                                    $pool | Set-Item
